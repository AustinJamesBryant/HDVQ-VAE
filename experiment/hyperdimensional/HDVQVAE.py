...
class HDVQVAE(nn.Module):
    ...
    def quantize(self, x):
        quantized, _, _ = self.vector_quantizer(x)
        return quantized
    ...
    def forward(self, x):
        z_e = self.encoder(x)
        quantized, perplexity_loss, loss = self.vector_quantizer(z_e)
        x_recon = self.decoder(quantized)
        return x_recon, perplexity_loss, loss
    ...
    def fit(self, train_dataloader, eval_dataloader, epochs, learning_rate=1e-3, device='cuda', save_dir='outputs'):
        ...
        for epoch in range(epochs):
            train_loss, train_mse, train_perplexity_loss, train_psnr_loss, train_ssim_loss, train_lpips_loss = self.process_epoch(train_dataloader, optimizer, device, train=True, lpips_network=lpips_network)
            eval_loss, eval_mse, eval_perplexity_loss, eval_psnr_loss, eval_ssim_loss, eval_lpips_loss = self.process_epoch(eval_dataloader, optimizer, device, train=False, save_images=True, epoch=epoch, save_dir=save_dir, lpips_network=lpips_network)
            current_lr = optimizer.param_groups[0]['lr']
            results[epoch] = {
                'train': {'loss': train_loss, 'mse_loss': train_mse, 'perplexity_loss': train_perplexity_loss, 'psnr': train_psnr_loss, 'ssim': train_ssim_loss, "lpips": train_lpips_loss},
                'eval': {'loss': eval_loss, 'mse_loss': eval_mse, 'perplexity_loss': eval_perplexity_loss, 'psnr': eval_psnr_loss, 'ssim': eval_ssim_loss, "lpips": eval_lpips_loss},
                'lr' : current_lr
            }

            scheduler.step(eval_loss)
            print(f"Epoch {epoch+1}/{epochs} - Train Loss: {train_loss:.3f}, MSE: {train_mse:.3f}, Perplexity Loss: {train_perplexity_loss:.3f}, PSNR Loss: {train_psnr_loss:.3f}, SSIM Loss: {train_ssim_loss:.3f}, LPIPS Loss: {train_lpips_loss:.3f}")
            print(f"Eval Loss: {eval_loss:.3f}, MSE: {eval_mse:.3f}, Perplexity Loss: {train_perplexity_loss:.3f}, PSNR Loss: {eval_psnr_loss:.3f}, SSIM Loss: {eval_ssim_loss:.3f}, LPIPS Loss: {eval_lpips_loss:.3f}")
    ...
    def process_epoch(self, dataloader, optimizer, device, train=True, save_images=False, epoch=None, save_dir='./', lpips_network=None):
        ...
        total_loss = total_mse = total_perplexity = total_psnr = total_ssim = total_lpips = count = 0
        ...
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Training" if train else "Evaluating")):
            ...
            with torch.set_grad_enabled(train):
                x_recon, perplexity_loss, loss = self.forward(inputs)
                ...
                total_loss += (mse_loss.item() + perplexity_loss.item() + psnr_loss.item() + ssim_loss.item() + lpips_loss.item())
        ...
        return avg_loss, avg_mse, avg_perplexity, avg_psnr, avg_ssim, avg_lpips
